...
class Trainer:
    ...
    def __init__(
        ...
        # Setup AMP scaler for mixed precision (only needed for FP16, not BF16)
    ...
    def _create_scheduler(self) -> optim.lr_scheduler._LRScheduler:
        ...
        total_steps = int(0.7 * len(self.train_dataloader) * self.config.max_steps)
    ...
    def training_step(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        with torch.cuda.amp.autocast(enabled=self.config.bf16 or self.config.fp16):
            outputs = self.model(
                input_ids=input_ids, attention_mask=batch.get("attention_mask")
            )
        ...
        if "labels" in batch and hasattr(batch["labels"], "min"):
            print(
                f"DEBUG: Labels range: {batch['labels'].min().item()} - {batch['labels'].max().item()}"
            )
        if self.config.fp16:
    ...
    def _evaluate(self) -> Dict[str, float]:
        ...
        for batch in self.eval_dataloader:
            ...
            with torch.cuda.amp.autocast(enabled=self.config.bf16 or self.config.fp16):
                ...
                shift_logits = logits[..., :-1, :].contiguous()
                shift_labels = (
                    batch["labels"][..., 1:].contiguous()
                    if "labels" in batch
                    else batch["input_ids"][..., 1:].contiguous()
                )
        ...
        self.logger.info(
            f"Validation - Loss: {avg_loss:.4f}, Perplexity: {perplexity:.4f}"
        )
        return metrics
