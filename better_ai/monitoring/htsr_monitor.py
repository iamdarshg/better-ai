...
def detect_correlation_traps(
    W: np.ndarray, n_random_trials: int = 10
) -> Tuple[bool, float]:
...
class LayerAlphaAnalyzer:
    ...
    def __init__(self, alpha_upper_threshold: float = ALPHA_OVER_GROKKING_THRESHOLD):
    ...
    def analyze_layer(
        self, layer_name: str, module: nn.Module
    ...
    def get_high_variance_layers(
        self, variance_threshold: float = ALPHA_VARIANCE_THRESHOLD
    ) -> Dict[str, float]:
...
class GrokkingDetector:
    ...
    def check(self, alpha_metrics: Dict[str, float]) -> Tuple[bool, Dict[str, Any]]:
        ...
        over_grokking_layers = {
            name: α
            for name, α in alpha_metrics.items()
        ...
        for name, alpha_val in alpha_metrics.items():
            if alpha_variance > 0:
                if np.abs(alpha_val - model_alpha) > 2 * np.sqrt(alpha_variance):
                    high_variance_layers[name] = alpha_val
...
class HTSRMonitor:
    ...
    def compute_all_layer_alphas(self) -> Dict[str, Any]:
        ...
        self.current_alpha_metrics = {
            name: result["alpha"] for name, result in all_results.items()
        }
        ...
        if details.get("is_over_grokking"):
            self.logger.severe(
                ...
                self.total_checks,
                {
                    ...
                    "over_grokking_layers": len(
                        details.get("over_grokking_layers", {})
                    ),
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "statistics": {
                ...
                "current_model_alpha": self.current_detector_state.get(
                    "model_alpha", 2.0
                ),
                "current_alpha_variance": self.current_detector_state.get(
                    "alpha_variance", 0.0
                ),
    ...
    def get_layer_summary(self) -> List[Dict[str, Any]]:
        ...
        for name, alpha in sorted(
            ...
            status = (
                "severe"
                if alpha > self.alpha_upper_threshold
                else "warning"
                if alpha > 3.5
                else "healthy"
            )
    ...
    def get_health_report(self) -> Dict[str, Any]:
        ...
        valid_alphas = [
            α for α in self.current_alpha_metrics.values() if α != float("inf")
        ]
        ...
        near_threshold = len(
            [
                α
                for α in valid_alphas
                if self.alpha_upper_threshold - 0.5 < α <= self.alpha_upper_threshold
            ]
        )
    ...
    def apply_intervention(
        ...
        if optimizer is not None:
            ...
            intervention_details["new_lrs"] = [
                pg.get("lr", 0) for pg in optimizer.param_groups
            ]
            intervention_details["previous_wds"] = current_wds
            intervention_details["new_wds"] = [
                pg.get("weight_decay", 0) for pg in optimizer.param_groups
            ]
