...
class EnhancedDeepSeekModel(nn.Module):
    ...
    def compute_trajectory_reward(
        ...
        ground_truth_answer: torch.Tensor
    ) -> torch.Tensor:
        """
        Compute reward for a reasoning trajectory

        Args:
            final_answer: The final answer generated by the model (batch_size, seq_len)
            ground_truth_answer: The ground truth answer (batch_size, seq_len)

        Returns:
            A reward tensor (batch_size,)
        """
        # For now, we use a simple reward: 1 if the final answer is correct, 0 otherwise.
        # This is calculated per-sample in the batch.
        is_correct = torch.all(torch.eq(final_answer, ground_truth_answer), dim=-1).float()
        return is_correct
