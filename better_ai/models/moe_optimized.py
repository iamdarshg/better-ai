...
class OptimizedExpertRouter(nn.Module):
    ...
    def __init__(
        ...
        self.router_linear = nn.Linear(
            hidden_size, num_experts, bias=router_bias, dtype=router_dtype
        )
    ...
    def update_load_stats(self, expert_counts: torch.Tensor):
        ...
        if self.update_counter % self.load_update_freq == 0:
            ...
            self.expert_loads_ema = (
                alpha * expert_counts + (1 - alpha) * self.expert_loads_ema
            )
...
class OptimizedMoELayer(nn.Module):
    ...
    def forward(
        ...
        if self.shared_experts > 0:
            ...
            for shared_idx in range(self.shared_experts):
                shared_expert_output = self.shared_experts_layer[shared_idx](
                    hidden_states_flat
                )
        ...
        if self.loss_free_balancing and self.balancer is not None:
            ...
            aux_losses = self._compute_aux_losses(
                router_logits, selected_experts_flat, total_tokens
            )
    ...
    def _compute_aux_losses(
        ...
        total_aux_loss = (
            self.load_balance_loss_weight * load_balance_loss + 0.001 * router_z_loss
        )
