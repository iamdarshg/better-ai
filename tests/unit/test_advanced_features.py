...
class TestAdvancedFeatures(unittest.TestCase):
    ...
    def test_specialized_head_creation(self):
        config = ModelConfig(use_json_db_ops_head=True, json_db_ops_ratio=0.2)
        ...
        config = ModelConfig(use_math_reasoning_head=True, math_reasoning_ratio=0.3)
        model = EnhancedDeepSeekModel(config)
        self.assertTrue(hasattr(model, 'math_reasoning_head'))
        self.assertEqual(model.math_reasoning_head.ratio, 0.3)

        config = ModelConfig(use_algorithm_head=True, algorithm_ratio=0.4)
        model = EnhancedDeepSeekModel(config)
    ...
    def test_head_forward_pass(self):
        config = ModelConfig(
            use_json_db_ops_head=True,
            use_math_reasoning_head=True,
            use_algorithm_head=True
        )
        model = EnhancedDeepSeekModel(config)
