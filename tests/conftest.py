...
def pytest_sessionfinish(session, exitstatus):
    ...
    Updates README.md with resource estimates based on config.py and datasets.yml.
    """
    import sys
    ...
    try:
        import update_readme_estimates

        # Run the update (only if configs changed)
        update_readme_estimates.update_readme(force=False)
    except Exception as e:
        # Don't fail tests if README update fails
        print(f"\n[conftest] Note: README estimate update skipped: {e}")
